...
They must sit in the `allwinner/` vendor subdirectory, not at the top level: since
6.12 the kernel's device tree makefiles build DTBs under their vendor path, so a
board names its device tree as `allwinner/<name>` in
