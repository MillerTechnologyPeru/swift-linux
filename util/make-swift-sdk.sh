...
if [ "$PORTABLE" = 1 ]; then
	...
	# usr/bin is excluded above (target binaries are useless on the host and
	# collide on a case-insensitive filesystem), but it has to EXIST: a CMake
	# package config generated by configure_package_config_file runs
	# set_and_check on its own bindir, and SDL2's aborts the configure of any
	# project that does find_package(SDL2) when the directory is missing.
	mkdir -p "$SYS/usr/bin"
